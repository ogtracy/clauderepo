...
-- ============================================================================
--
-- This script creates the main tables for storing Open Library data:
--   1. work_creator   (authors)
--   2. quillent_work  (works)
--   3. work_editions  (editions)
--   4. search_tag     (tags extracted from works subjects)
--   5. work_tags      (junction table: works ↔ tags)
--
...
DROP TABLE IF EXISTS search_tag CASCADE;
...
-- Table: search_tag
-- ============================================================================
-- Stores unique tags extracted from Open Library work subjects
-- Source: Derived from works subjects field via process_tags.py
-- ============================================================================

CREATE TABLE search_tag (
    -- Primary key: auto-incrementing ID (but pre-assigned in CSV)
    id SERIAL PRIMARY KEY,

    -- Tag key (e.g., "/tags/science-fiction")
    uuid VARCHAR(255) NOT NULL UNIQUE,

    -- Tag display name (lowercase, cleaned)
    tag_name VARCHAR(100) NOT NULL UNIQUE,

    -- Prevalence: number of works with this tag
    prevalence BIGINT DEFAULT 0,

    -- Weight: quality/relevance score (inverse document frequency)
...
-- Indexes for search_tag
CREATE INDEX idx_search_tag_tag_name ON search_tag(tag_name);
CREATE INDEX idx_search_tag_prevalence ON search_tag(prevalence DESC);

COMMENT ON TABLE search_tag IS 'Unique tags extracted from Open Library work subjects';
COMMENT ON COLUMN search_tag.uuid IS 'Tag URI (e.g., /tags/science-fiction)';
COMMENT ON COLUMN search_tag.tag_name IS 'Cleaned, lowercase tag display name';
COMMENT ON COLUMN search_tag.prevalence IS 'Number of works tagged with this tag';
COMMENT ON COLUMN search_tag.weight IS 'Inverse document frequency weight (0-1)';

-- ============================================================================
-- Table: work_tags
-- ============================================================================
-- Junction table for many-to-many relationship between works and tags
-- Source: Generated by process_tags.py
-- ============================================================================

CREATE TABLE work_tags (
    -- Foreign key to quillent_work
    work_id INTEGER NOT NULL,

    -- Foreign key to search_tag
    ...
    CONSTRAINT fk_work_tags_work FOREIGN KEY (work_id)
        REFERENCES quillent_work(id) ON DELETE CASCADE,
    CONSTRAINT fk_work_tags_tag FOREIGN KEY (tag_id)
        REFERENCES search_tag(id) ON DELETE CASCADE
);

-- Indexes for work_tags
CREATE INDEX idx_work_tags_work_id ON work_tags(work_id);
CREATE INDEX idx_work_tags_tag_id ON work_tags(tag_id);

COMMENT ON TABLE work_tags IS 'Junction table linking works to tags (many-to-many)';
COMMENT ON COLUMN work_tags.work_id IS 'FK to quillent_work.id';
COMMENT ON COLUMN work_tags.tag_id IS 'FK to search_tag.id';
...
CREATE TRIGGER update_search_tag_updated_at BEFORE UPDATE ON search_tag
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
--   SELECT * FROM search_tag ORDER BY prevalence DESC LIMIT 10;
--   SELECT * FROM work_tags LIMIT 10;
--
-- Tag statistics:
--   -- Most common tags
--   SELECT tag_name, prevalence FROM search_tag ORDER BY prevalence DESC LIMIT 20;
--
--   -- Tags per work distribution
--   SELECT COUNT(*) as tag_count, COUNT(DISTINCT work_id) as num_works
--   FROM work_tags GROUP BY work_id;
--
-- Table sizes:
--   SELECT
--       schemaname,
...
--   FROM pg_tables
--   WHERE tablename IN ('work_creator', 'quillent_work', 'work_editions', 'search_tag', 'work_tags')
